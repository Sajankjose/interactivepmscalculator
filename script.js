...
function calculateResults() {
    ...
    // Arrays to hold data for visualization
    let yearLabels = [];
    let yearEndNavData = [];
    let highWatermarkData = [];
    let fixedFeeData = [];
    let otherExpensesData = [];
    let performanceFeeData = [];
    ...
    for (let i = 1; i <= period; i++) {
        ...
        // Push data for chart visualization
        yearLabels.push(`Year ${i}`);
        yearEndNavData.push(yearEndNav);
        highWatermarkData.push(highWatermark);
        fixedFeeData.push(fixedFee);
        otherExpensesData.push(otherExpenses);
        performanceFeeData.push(performanceFee);
    ...
    renderChart(yearLabels, yearEndNavData, highWatermark
